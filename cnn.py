...
class CNN(object):
    ...
    def append_conv2d_layer(self, num_of_filters, kernel_size=3, padding='same', strides=1,
        ...
        """
         ...
         :return: Layer object
         """
        return self.model.add(keras.layers.Conv2D(num_of_filters, kernel_size, strides=strides,
    ...
    def get_weights_without_biases(self, layer_number=None, layer_name=""):
        ...
        if layer_number == None:
            m = [layer.name for layer in self.model.layers]
            if layer_name in m:
                n = self.model.get_layer(name=layer_name)
                if len(n.get_weights()) > 0:
                    return n.get_weights()[0]
    ...
    def get_biases(self, layer_number=None, layer_name=""):
        ...
        if layer_number == None:
            ...
            return self.model.get_layer[layer_number].get_weights()[1]

        elif layer_number > 0:
            if len(self.model.get_layer(index=layer_number - 1).get_weights()) > 0:
    ...
    def set_weights_without_biases(self, weights, layer_number=None, layer_name=""):
        ...
        if layer_number == None:
            ...
            if layer_name in m:
                if len(self.model.get_layer(name=layer_name).get_weights()) > 0:
                    bias = self.model.get_layer(name=layer_name).get_weights()[1]
                    new_weights = [weights, bias]
                    self.model.get_layer(name=layer_name).set_weights(new_weights)
        elif layer_number != 0:
            self.model.layers[layer_number - 1].get_weights()
            m = self.model.get_layer(index=layer_number - 1)
            if len(m.get_weights()) > 0:
                bias = self.model.get_layer(index=layer_number - 1).get_weights()[1]
                new_weights = [weights, bias]
                self.model.get_layer(index=layer_number - 1).set_weights(new_weights)
    ...
    def set_biases(self, biases, layer_number=None, layer_name=""):
        ...
        if layer_number == None:
            ...
            if layer_name in m:
                if len(self.model.get_layer(name=layer_name).get_weights()) > 0:
                    weights = self.model.get_layer(name=layer_name).get_weights()[0]
                    new_bias = [weights, biases]
                    self.model.get_layer(name=layer_name).set_weights(new_bias)
        elif layer_number != 0:
            if len(self.model.get_layer(index=layer_number).get_weights()) > 0:
                weights = self.model.get_layer(index=layer_number - 1).get_weights()[0]
                new_bias = [weights, biases]
                self.model.get_layer(index=layer_number - 1).set_weights(new_bias)
    ...
    def load_a_model(self, model_name="", model_file_name=""):
        ...
        model_1 = None
        if model_name == "VGG16":
            model_1 = keras.applications.VGG16()
        elif model_name == "VGG19":
            model_1 = keras.applications.VGG19()
        if model_1:
            self.model = keras.models.Sequential()
    ...
    def train(self, X_train, y_train, batch_size, num_epochs,optimizer, loss, metric):
        """
        ...
        history = self.model.fit(x=X_train, y=y_train, epochs=num_epochs, batch_size=batch_size)
